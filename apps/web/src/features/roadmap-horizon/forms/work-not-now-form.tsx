...
export default function WorkNotNowForm({
	...
	const [grounds, setGrounds] = useState<
		Array<{ id: string; kind: GroundKind }>
	>([]);
	const [reviewLaterEffect, setReviewLaterEffect] = useState<
		typeof ROADMAP_COPY.keepReviewLater | typeof ROADMAP_COPY.removeReviewLater
	...
	const onReconsider = useCallback(() => {
		...
		result.value.catch(() => undefined);
	}, [attemptOnlineWork, markUnsaved, reconsider, reviewLaterEffect, workId]);
	const active = trail.data?.active ?? null;
	const history = trail.data?.history ?? [];
	const open = workStatus !== "Closed";
	return (
				...
				<form className="flex flex-col gap-3" onSubmit={onApply}>
					...
					{grounds.length > 0 ? (
						<ul className="text-muted-foreground text-xs">
							{grounds.map((ground) => (
								<li key={`${ground.kind}:${ground.id}`}>
									{ground.kind} · {ground.id}
								</li>
							))}
						</ul>
					) : null}
					{preview.data ? (
						<div className="border px-3 py-2 text-sm">
							<p>
								{ROADMAP_COPY.preview}: {preview.data.reason}
							</p>
							{preview.data.reevaluationCondition ? (
								<p>{preview.data.reevaluationCondition}</p>
							) : null}
							<p>{preview.data.reviewLater.effect}</p>
						</div>
					) : null}
					<ReviewLaterEffectField
						id={`${workId}-not-now-review-later`}
						onChange={setReviewLaterEffect}
				...
				<div className="flex flex-col gap-2">
					{reconsiderPreview.data ? (
						<p className="text-sm">
							...
							{reconsiderPreview.data.reviewLater.effect}
						</p>
					) : null}
					<ReviewLaterEffectField
						...
						value={reviewLaterEffect}
					/>
					<Button onClick={onReconsider} size="sm" type="button">
