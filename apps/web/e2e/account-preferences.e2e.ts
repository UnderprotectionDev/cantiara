...
async function replaceWithKeyboard(field: Locator, value: string) {
  await field.focus();
...
test("keeps browser suggestions unsaved and persists Account Preferences on Save", async ({
  ...
  await page.getByLabel("Time zone").fill("America/");
  await expect(
    page.getByRole("heading", { name: "Preview", level: 2 }),
  ).toBeVisible();
  await page.getByLabel("Time zone").fill("Europe/Istanbul");
  ...
  await replaceWithKeyboard(page.getByLabel("Locale"), "tr-TR");
  await replaceWithKeyboard(
    page.getByLabel("Time zone"),
    "America/Los_Angeles",
  );
  await page.getByLabel("Date format").focus();
  await page.getByLabel("Date format").pressSequentially("2026-12-31");
  await page.getByLabel("First day of week").focus();
  await page.getByLabel("First day of week").press("s");
  await page.getByRole("combobox", { name: "Appearance" }).focus();
  await page.getByRole("combobox", { name: "Appearance" }).press("l");
