...
Only the backend accesses Postgres. It owns Drizzle client and schema at `src/db/index.ts` and `src/db/schema.ts`, config at `drizzle.config.ts`, and generated SQL migrations under `drizzle/`. Prisma is not used.
