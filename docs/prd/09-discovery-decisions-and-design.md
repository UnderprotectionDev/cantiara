...
- **Kullanıcı mevcut ana Kullanıcı Akışı düğümü, Ekranın kesin Wireframe sürümündeki block veya desteklenen kesin görsel konumunda ortak `Kayda dönüştür ve bağla` eylemini başlatabilir.** Akış tam olarak bir yeni İş, Karar, Risk veya Açık Soru taslağı oluşturur; uygulamadan önce hedef tür ve proje, başlangıç başlık/gövde eşlemesi, oluşacak `Kökeni` ilişkisi ve değişmez `Köken konumu` (sahip Ekran veya Kullanıcı Akışı kimliği, düğüm/block kimliği, kesin Wireframe veya akış sürümü) gösterilir. Kullanıcı taslağı düzenleyip onaylamadan ana kayıt oluşmaz; kaynak tasarım öğesi yerinde kalır, bağımsız ilişki ucu olmaz ve görünümü, semantiği veya içeriği değiştirilmez.
