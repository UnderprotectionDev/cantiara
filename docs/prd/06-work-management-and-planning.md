...
- **Kullanıcı dış çalışma döndüğünde yürütücünün özetini, değişen varsayımları, üretilen kanıt veya izinli dış bağlantıları ve kapanmamış soruları aynı devre kaydedip `Uzlaştır` eylemini başlatır.** Uzlaştırma önizlemesi yalnız aynı Projedeki başka arşivlenmemiş İş kayıtlarına kurulacak desteklenen kesin ilişkileri ve aynı Projede açıkça oluşturulacak takip İşlerini gösterir. Bu uzlaştırma Decision, Risk, Document veya diğer ana kayıt türlerini Relations uçları olarak eklemez. Devir metni kendiliğinden Karar, Risk, İş, ilişki veya kanıt üretmez; kullanıcı onayı sonucu ve seçilen bağları tarihsel uzlaştırma kararıyla kaydeder.
