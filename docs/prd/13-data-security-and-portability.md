...
- **Çöp Kutusu kayıtları 30 gün sonra kalıcı silinir.** Kullanıcı daha önce açık kalıcı silme başlatabilir; etki önizlemesi, [GitHub kimliğini yeniden teyit etme](03-account-platform-operations.md#github-kimliğini-yeniden-teyit-etme) ve etkilenen Hesap ya da Proje adını yazan açık onay gerekir. Bu teyit parola/MFA girişi olarak sunulmaz. Kimlik doğrulama, yetkilendirme, paylaşım, yayın, entegrasyon ve yüksek riskli veri işlemi denetim kayıtları 365 gün; olağan güvenlik olay logları 90 gün; diğer uygulama ve operasyon logları 30 gün tutulur. Restore replay için ayrı tutulan içeriksiz ve geri döndürülemez takma kimlikli silme/iptal/rotasyon tombstone'u, ilgili Hesap silinse bile en uzun operasyonel backup/restore penceresi artı 30 gün korunur ve sonra fiziksel silinir. Secret ve özel içerik hiçbir log veya tombstone'a yazılmaz. Güvenlik redaksiyonu ve kalıcı silme bu sürelerden bağımsız açık yaşam döngüleridir.

- **Kalıcı silinen GitHub dış kaydının otomatik yeniden oluşmasını engelleyen tombstone yalnız sağlayıcıyı, kararlı repository ve kaynak kimliğini, silme zamanını ve geri döndürülemez yerel takma kimliği taşır; başlık, gövde, URL veya başka kaynak içeriği taşımaz.** Tombstone kanonik Proje yaşadığı sürece korunur. Kullanıcının kaynağı açıkça yeniden dahil etmesi tombstone'u kaldırmaz; yeni ürün kimliğini eski silme kökeninden ayırır. Proje veya Hesap kalıcı silindiğinde tombstone restore replay için en uzun operasyonel backup/restore penceresi artı 30 gün tutulur ve ardından fiziksel silinir.
...
- **Proje silme grubunun hangi kayıtları kapsadığı ve nasıl geri yüklendiği [ortak yaşam döngüsünde](02-domain-model-and-lifecycle.md#ortak-yaşam-döngüsü) tanımlanır.** Güvenlik açısından grup 30 günlük geri alınabilir süreyi tek birim olarak izler; çocuk kayıtlar ayrı sürede kalıcı silinemez. Süre sonu veya GitHub kimliği yeniden teyit edilmiş erken kalıcı silme bütün gruba uygulanır ve içerikleri kaldırır; zorunlu içeriksiz kimlik ile denetim işaretleri kalabilir.
...
- **JSON, aşağıdaki kapalı katalogdaki seçili kayıtların kanonik yapılandırılmış round-trip biçimidir.** Güncel alan değerlerini, kararlı kimlikleri ve İş anahtar geçmişini, seçili kapsamdaki özel alan tanımlarını, kökeni ve iki ucu da seçilmiş ilişkileri taşır; olağan kayıt geçmişini taşımaz. Her çıktı açık şema sürümü taşır. Ürün kendi yayımladığı v1'den itibaren içe aktarılabilir kayıt türlerinin bütün JSON şema sürümlerini test edilmiş migration'larla destekler; bilinmeyen gelecek sürüm hiçbir yazma yapmadan reddedilir. Kaçınılmaz veri kaybı varsa import önizlemesinde alan bazında açıklanır. Teknik Diyagramın ilk üründeki export-only istisnası aşağıdaki kendi sözleşmesinin sahibidir ve bu genel round-trip vaadini miras almaz.

| Genel CSV/JSON içe aktarma kayıt ailesi | İlk ürün sınırı |
| --- | --- |
| İş | `Feature`, `Bug`, `Task`, `Research`, `Improvement` |
| Planlama | Proje Hedefi, Kilometre Taşı |
| Karar ve belirsizlik | Karar, Risk, Varsayım, Açık Soru |
| Girdi ve kişi bağlamı | Geri Bildirim, Contact, Company |
| Ürün öğrenimi | Ürün Boşluğu, Kullanıcı Araştırması Oturumu, Deney/Doğrulama |
| Dış bilgi | Yalnız Kaynak üstverisi; yakalanmış tarihsel içerik ve sürüm gövdesi dış dosyadan güvenilir Kaynak snapshot'ı olarak alınmaz |
| Sürüm ve işletim | Proje Sürümü, Üretim Olayı |

- **Belge genel CSV kataloğunda değildir; tekil Markdown veya yalnız Cantiara'nın yayımladığı sürümlü Belge JSON'u ile alınır.** Planlı veya bildirilen test verisi genel katalogdan değil `test-report/1` ve Test alanının kendi sözleşmesinden; Dosya Eki özgün dosya yüklemesinden geçer. Teknik Diyagram JSON'u export-only kalır. Ekran/Wireframe, Kullanıcı Akışı, Proje Duvarı ve Moodboard genel CSV/JSON import'una girmez. GitHub dış kayıtları dosyadan GitHub gerçeği olarak oluşturulamaz. Proje, Dış yüzey, otomasyon, Başlangıç yapılandırması ve diğer yapılandırma varlıkları genel import'la oluşturulmaz.
...
- **Çıktı, seçili kayıt taşınabilirliğidir; [şifreli Çalışma Alanı çıkış paketi](#calisma-alani-cikis-paketi) ayrı sözleşmedir.** Hesap kapatma sırasında ürün tam çıkış paketini üretir; seçili belge/görünüm/ek export'ları da kullanılabilir kalır. İşlem mevcut eşzamanlı dışa aktarma sınırını aşarsa sessizce uzun süre çalışmaz.

### Standart dosyalardan içe aktarma

- **Kullanıcı tekil Markdown dosyasını Proje veya Wiki Belgesi olarak; Cantiara'nın yayımladığı sürümlü tekil Belge JSON'unu aynı kayıt ailesi olarak; tekil CSV ya da JSON dosyasını yukarıdaki kapalı katalogdan dosya başına tek kayıt türü ve tek seçili kapsam için içe aktarabilir.** Bir işlem en fazla 10.000 satır veya 25 MB'dir; hangisine önce ulaşılırsa sınır odur ve daha büyük veri açıkça bölünür. İlk ürün klasör, ZIP, çok dosyalı Markdown paketi veya bütün Çalışma Alanı import'u yapmaz. Hedef kapsam, oluşturulacak/güncellenecek kayıtlar, kaynak alanların ürün alanlarına ve Proje bazlı özel alanlara dönüşümü, kayıp ve çakışmalar uygulanmadan önce gösterilir.
...
- **Ürün otomatik yedek zaman çizelgesi, restore-point kütüphanesi veya ürün içine atomik geri yükleme sunmaz.** [Çalışma Alanı çıkış paketi](#calisma-alani-cikis-paketi) taşınabilir şifreli arşivdir; yedek ürünü veya restore vaadi değildir. Standart Markdown/JSON/CSV dışa aktarımları seçili verinin taşınması içindir. Hizmetin zorunlu operasyonel yedek davranışı yalnız [operasyonel yedek ve kurtarma](03-account-platform-operations.md#operasyonel-yedek-ve-kurtarma) bölümündedir.

<a id="calisma-alani-cikis-paketi"></a>
### Çalışma Alanı çıkış paketi

- **Kullanıcı yaşayan Hesaptan güvenlik/taşınabilirlik yüzeyinde, ve Hesap kapanışında zorunlu olarak, tam Çalışma Alanı çıkış paketi üretebilir.** Paket kullanıcı içeriğini, ek sürümlerini, kimlikleri, ilişkileri, geçmişi ve yapılandırmaları okunabilir Markdown/JSON ve özgün binary'lerle tek manifestli arşivde taşır. Oturumlar, secret'lar, dış erişim anahtarları, operasyon logları, türetilmiş cache'ler ve Yakalama staging ekleri girmez.

- **Paket kullanıcı parolasıyla zorunlu şifrelenir; parola sunucuda saklanmaz.** Unutulan parola paketi okunamaz kılar; ürün kurtarma kodu veya şifresiz kopya sunmaz. Uygulamasız doğrulanabilir envelope tanımı yayımlanır. İndirme şifreli, süreli ve iptal edilebilirdir; yaşayan Hesapta restore-point kütüphanesi oluşmaz.

- **Kapanış dondurması en az bir başarılı paket üretimini gerektirir ve paketi 30 gün indirilebilir tutar.** Kalıcı silme bu üretimden önce olmaz. Seçili Markdown/JSON/CSV export durur. Ürün içine restore/import [gelecek yönündedir](18-future-directions.md#tam-ürün-paketi-ve-geri-yükleme-doğrulaması).
