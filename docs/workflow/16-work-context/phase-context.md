...
`Bağlamı Markdown kopyala` iş anahtarı, başlık, tür, durum, açıklama, kontrol listesi, neden zinciri, Birincil spec, ilgili belirsizlik ve izinli dış bağlantıları okunabilir Markdown olarak panoya aktarır.

Çıktı üretim zamanını, okunabilir kaynak kimliklerini ve `Ana kaynak uygulamadadır` notunu taşır. Yeni kayıt veya kalıcı snapshot oluşmaz. Secret, erişilemeyen alan veya özel ek içeriği kapsama girmez.

Kart özel iç çalışma düzenidir. Görünen bölüm bağlantıyla paylaşım veya Build in Public kapsamı açmaz.

### Öncelik dayanakları

`Öncelik dayanakları` hedef, tarih, blokaj, risk, kilometre taşı, geri bildirim ve ölçütleri kaynaklarına bağlı ve taranabilir biçimde toplar. Sayısal skor, otomatik sıra hükmü veya nesnel öncelik iddiası üretmez; işin önceliğini kullanıcı belirler.

Desteklenen bağlam türleri için tıklanabilir kayıt sayıları kesin filtrelenmiş kümeyi açar. Geri Bildirim sayıları kayıt, benzersiz Contact ve varsa benzersiz Company olarak karışmadan durur. Arşiv kayıtları sayıya dahildir; Çöp Kutusu ve kalıcı silinenler değildir.

Bu alt faz Önceliklendirme oturumu veya Backlog sırası değildir. Kararı destekleyen okuma özetidir.
...
- Markdown kopyası yeni kayıt veya kalıcı snapshot üretmez.
- Öncelik dayanakları otomatik skor veya sıra hükmü üretmez.
