...
- **Evidence module.** `Kanıtı` / `Kanıt sağlar` ends per PRD 02: exact Source/Document/Diagram version, Feedback, Research Session, Validation Record, Session Test, or File Attachment version → Work/Decision/Risk/Assumption/Question/Test/Project Release or Access/Outcome observation. Pin API used by 43/44 rebind. Role + interpretation on the relation helper entity.
