...
16. As a founder who opts a Project in, I want one `reappear-date` `Action Required` signal when the date arrives, so that opt-in is per Project, not global spam.
...
- **Glossary.** Use Backlog, Akıllı Koleksiyon (Backlog is a prepared one), Backlog manuel sırası, Backlog kayıtlı sunumu, Yeniden görünme tarihi (`Reappear date`), Deferred, Günlük Odak (candidate, not membership), Kanban (must not receive this rank), Önceliklendirme oturumu (must not write this rank). Do not introduce folder, tag, static list, sprint, or Kanban order.
...
- **One saved alternative presentation.** A Project may save one Priority (with an explicit criterion), Date, or Field (with an explicit field) presentation and select it again after leaving Backlog. This saved choice is separate from the Project's manual Work order and does not write that order when saved or selected.
...
- **Notification opt-in.** `reappear-date` is off by default. Only an explicit per-Project `Notify on Reappear date` opt-in produces the `Action Required` signal when the Account-local date arrives; enabling after that local date does not backfill old dates. One signal is stored per Work and Reappear date, with a link to its source Work; a repeated scan does not duplicate it. The notification center displays it; this feature does not build the center. Personal `Review later` reminders are the personal-reminders feature and are not this date.
- **English UI labels.** First user-visible copy uses: `Backlog`, `Manual order`, `Deferred`, `Reappear date`, `Notify on Reappear date`. The order presentation control is `Backlog sort` with `Manual order`, `Priority`, `Date`, and `Field`; `Priority criterion` selects the criterion to sort by and `Field to sort by` selects `Title` or `Status`. The saved choice uses `Saved presentation`, `Save presentation`, `Use saved presentation`, and `Saved presentation: {sort}[ · {detail}]` (detail appears for Priority and Field); a failed save shows `Backlog presentation could not be saved. Try again.` The reorder handle is `Drag {Work title}` and its live announcements name the Work during pickup, movement, drop, and cancellation. A failed reorder shows `Backlog order could not be saved. Try again.` The date picker uses `Reappear date` and `Clear` to set or remove the date. A failed date edit shows `Reappear date could not be saved. Try again.` The prepared Backlog query states use `Loading Backlog…`, `Backlog is unavailable. Try loading this page again.`, and `No active Work to consider.` Missing labels are added to the PRD term table in the same change that first shows them. No Turkish UI.

## Testing Decisions

- **What a good test is.** Tests observe Backlog through its public interface: prepared membership, status-unchanged on view/pick-up, persist/restore of manual order across explicitly chosen priority criterion, date, and field sorts, save and reselect the one Project presentation after returning to Backlog, keyboard reorder announcements, failed reorder feedback, `Deferred` split, date arrival restoring rank with an Account time zone clock double, date edits preserving Work status and project membership, failed date edit feedback, the default-off, prior Project opt-in, and late opt-in matrix producing exactly one source-linked `reappear-date` only for timely opt-in without changing Work status, priority, or Backlog order, and the exact accessible loading, unavailable, and empty copy for the prepared collection query. They do not assert sort-index internals. Expected values are product rules (membership does not write status; one persistent order; future date is Deferred, not a status; query state copy remains secret-free and does not imply a write).
