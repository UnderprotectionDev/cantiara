...
- **Notification opt-in.** `reappear-date` is off by default. Only an explicit per-Project `Notify on Reappear date` opt-in produces the `Action needed` signal when the Account-local date arrives; enabling after that local date does not backfill old dates. One signal is stored per Work and Reappear date, with a link to its source Work; a repeated scan does not duplicate it. The notification center displays it; this feature does not build the center. Personal `Review later` reminders are the personal-reminders feature and are not this date.
