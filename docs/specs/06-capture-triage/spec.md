...
- **Glossary.** Use Yakalama Gelen Kutusu öğesi, Çalışma alanı Yakalama Gelen Kutusu (`Workspace Capture Inbox`), Proje Yakalama Gelen Kutusu (`Project Capture Inbox`), Yakalama mini şablonu, Sıralı triage (`Sequential triage`), Toplu Anlamlandırma (`Bulk sense-making`), Yakalama eki, Web Yakalama, Taslak (must not be this), İş (must not be this), Dosya Eki (must not be the staging object). Do not introduce bookmark, SLA queue, or auto-triage.
- **Capture Inbox module.** Workspace Inbox and per-Project Inbox. The Capture Inbox page lists every Inbox that has items, grouped as `Workspace Capture Inbox` and each `Project Capture Inbox`. Save with Project empty writes the Workspace Inbox; Save with Project set writes that Project Inbox. The `Project` control lists available Projects by visible name and short code; the selected internal Project id is submitted without asking the founder to type it. The empty selection is `Workspace Capture Inbox`. Project copy says `Leave empty to save to the Workspace Capture Inbox.` An empty page shows `No captures in this Inbox.` A Project Inbox lists and groups items without requiring the same capital letters. Items are temporary: no main identity, lasting relation, archive, search, share, publish, or export. They do not auto-delete on a timer. Human writes use Mutation Contract. Direct `Create Bug` (when Project and type are known) calls Work create and writes no Inbox item — this module does not assign Work keys. `Create Bug` is available when Project is set and type is Bug Capture or unspecified; the seam refuses Feedback Capture and Research Fragment without a receipt. Otherwise the UI says `Create Bug is available when Project is set and type is Bug Capture.` The hand-off copy is `Create Bug does not stay in the Capture Inbox. A Work record is not stored yet.`
...
- **Triage exits.** Exactly three: convert to one new main record (hand off to the owning feature; Markdown Document is one allowed target; no multi-record recipe); bind/merge to an existing record as origin or evidence (evidence *link type* remains workflow 45’s specialist; this feature may attach origin and must preview); delete. Consume the Inbox item. Origin trace is derived later on the record; this feature preserves origin fields. Convert preview shows original text/link/screenshot beside the proposed record and, for mini templates, field mappings before apply. A Workspace Capture Inbox item converted to Work requires a target Project and shows that target scope in the preview; a Project Capture Inbox item keeps its immutable Project scope. Same-Project similar suggestions are primary; other-Project matches are a named secondary group with Project names; bind still requires target+relation preview. Merge undo uses Mutation Contract: preview restores the original Inbox item (message, link, attachment, capture time, origin) and removes only binds/fields this merge wrote; later unrelated target edits stay.
...
- **English UI labels.** First user-visible copy uses: `Capture Inbox`, `Workspace Capture Inbox`, `Project Capture Inbox`, `Leave empty to save to the Workspace Capture Inbox.`, `Work conversion requires a Project.`, `Sequential triage`, `Bulk sense-making`, `Ungrouped`, `Bug Capture`, `Feedback Capture`, `Research Fragment`, `Create Bug`, `Convert`, `Attach to existing`, `Delete`, `No captures in this Inbox.` Sequential triage and Bulk sense-making are already glossary UI; add the others to the PRD term table in the same change that first shows them.
- **Stack.** Web app + Hono/oRPC + Drizzle + R2 for staging blobs with envelope encryption; WXT extension; Uppy only if upload reuse fits without becoming Dosya Eki. Do not add an offline extension queue or a second clip archive.

## Testing Decisions

- **What a good test is.** Tests observe Capture Inbox through its public interface: freeform and mini-template save without a main record; `Create Bug` when Project+type known creates no Inbox item; the `Project` control offers `Workspace Capture Inbox` and authorized Projects as visible name plus short code while submitting the internal Project id; empty Inbox copy `No captures in this Inbox.`; the page lists Workspace and each Project Inbox together; list heading `Workspace Capture Inbox` or `Project Capture Inbox`; Project Inbox listing and page grouping without requiring the same capital letters; `Create Bug` unavailable unless Project is set and type is Bug Capture or unspecified — the Capture Inbox seam refuses Feedback Capture and Research Fragment without a receipt; Workspace-to-Work conversion requires a selected Project and previews its target scope; converted Work retains original capture content, fields, link, attachment, capture time, template, and origin; conversion retry reuses one idempotency key; three exits consume the item; convert preview before apply; merge undo restores Inbox fields without rewinding later target edits; Sequential does not advance on view/edit; Bulk cluster is not a tag and disappears on resolve; Bulk placement options are `Ungrouped` plus each named cluster; staging object absent from search/export; extension retry is idempotent; unpaired or 30-day-stale client cannot write; disconnect creates no queue. They do not assert R2 keys, WXT boilerplate, or Drizzle shapes. Expected values are product rules (three exits, optional fields, no timer delete, no Safari requirement).
