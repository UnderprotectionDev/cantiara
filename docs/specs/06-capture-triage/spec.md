...
- **Capture Inbox module.** Workspace Inbox and per-Project Inbox. The Capture Inbox page lists every Inbox that has items, grouped as `Workspace Capture Inbox` and each `Project Capture Inbox`. Save with Project empty writes the Workspace Inbox; Save with Project set writes that Project Inbox. Project copy says `Leave empty to save to the Workspace Capture Inbox.` An empty page shows `No captures in this Inbox.` A Project Inbox lists without requiring the same capital letters. Items are temporary: no main identity, lasting relation, archive, search, share, publish, or export. They do not auto-delete on a timer. Human writes use Mutation Contract. Direct `Create Bug` (when Project and type are known) calls Work create and writes no Inbox item — this module does not assign Work keys. `Create Bug` is available when Project is set and type is Bug Capture or unspecified; otherwise the UI says `Create Bug is available when Project is set and type is Bug Capture.` The hand-off copy is `Create Bug does not stay in the Capture Inbox. A Work record is not stored yet.`
