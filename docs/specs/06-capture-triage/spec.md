...
- **English UI labels.** First user-visible copy uses: `Capture Inbox`, `Workspace Capture Inbox`, `Project Capture Inbox`, `Leave empty to save to the Workspace Capture Inbox.`, `Sequential triage`, `Bulk sense-making`, `Ungrouped`, `Bug Capture`, `Feedback Capture`, `Research Fragment`, `Create Bug`, `Convert`, `Attach to existing`, `Delete`, `No captures in this Inbox.` Sequential triage and Bulk sense-making are already glossary UI; add the others to the PRD term table in the same change that first shows them.
