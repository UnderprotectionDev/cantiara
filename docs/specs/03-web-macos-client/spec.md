...
26. As a founder using only a keyboard, I want to skip repeated app navigation and move focus to the current page content, so that I can enter a task without tabbing through the full shell.
...
- **Support reference.** Every failed main flow that reaches the server shows a clear reason, a safe retry bound, whether data was written, and a server-issued support reference with no secret and no private content. A flow that never reaches the server shows `Support reference unavailable.` and never fabricates a client reference. An unwritten-write toast stays until the founder dismisses it or chooses Retry, so Retry and the Support reference remain reachable. Query/list failures and already-written failures auto-dismiss. This is not a pager, S1 alarm, or operator runbook. Better Stack remains the operator sink; the user never sees tokens, emails-as-secrets, or Workspace bodies.
- **English UI labels.** First user-visible copy uses: `You’re offline`, `Last saved`, `Unsaved changes may be lost`, `Update required`, `Support reference`, `Support reference unavailable.`, `Cantiara couldn’t be reached.`, `Cantiara couldn’t check your session.`, `Data was written.`, `Data was not written.`, `Data write outcome is unknown.`, `Retry`, `Skip to main content`. That spec English is the name in code and in the interface.
- **Skip navigation.** `Skip to main content` focuses the current page content without navigating to a fragment. On a Project surface, it preserves the active surface in the URL hash.
...
- **What a good test is.** Tests observe Client Shell through its public interface: disconnect shows last save and refuses writes without a queue; `Unsaved changes may be lost` appears only when the unsaved-risk flag is set and is omitted after a successful save that left nothing unsaved; reconnect does not flush a hidden queue; updater accepts a valid signature and rejects an invalid one without breaking the previous app; a client past the 30-day window is stopped before a write; a failed flow returns a support reference that contains no token, session, or private field; a flow that never reaches the server shows `Support reference unavailable.` without fabricating a client reference; an unreachable protected-route session check shows `Cantiara couldn’t be reached.` and `Support reference unavailable.`, hides raw transport errors including browser-specific network messages, and offers `Retry` to repeat the check; a server-side session-check error shows `Cantiara couldn’t check your session.` with its server-issued Support reference instead of treating the founder as signed out and offers `Retry`; an unwritten-write Retry toast stays until dismissed; an unknown write outcome shows `Data write outcome is unknown.` and offers no Retry; a failed Inbox list and an already-written failure auto-dismiss; a PostgreSQL `42P01`/`42703` list failure (missing table or column) still shows a secret-free reason and `Data was not written` instead of collapsing newlines into the generic failed copy; a query referencing a field absent from the current Drizzle schema maps to restart-after-migrate copy; a missing current-schema gate uses the same copy; in development a schema fingerprint mismatch applies pending Drizzle migrations before the write, and the write is retried once after refreshing the database connection so a just-migrated schema can save without a founder restart; an unmatched RPC (`Not Found` / `404 Not Found`) presents restart-the-API copy instead of HTTP jargon, and an unmatched `/rpc` response still issues a secret-free Support reference; keyboard users can bypass the app header with `Skip to main content`, focus current page content, and preserve the selected Project surface hash. They do not assert Tauri internals, Railway topology, or Better Stack payloads. Expected values are product rules (no queue, flag-driven unsaved-risk copy, 30-day window, secret-free reference, transport failures do not fabricate a reference, unknown write outcomes do not retry, unwritten-write toast stays until dismissed, query/list and written toasts auto-dismiss, schema-drift list failures stay readable and secret-free, unmatched RPC is not HTTP jargon, and the focused skip target is the current page content without changing navigation state).
...
- **Keyboard navigation path.** On `/projects`, the first `Tab` reaches `Skip to main content`. On a Project `Work` surface, activating that control focuses current page content while preserving the `#work` surface.
