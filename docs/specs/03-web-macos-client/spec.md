...
- **Owning documents.** Behavior is owned by [çalışma ve dağıtım modeli](../../prd/03-account-platform-operations.md#calisma-ve-dagitim-modeli). User-visible error copy is the client half of [hizmet işletimi](../../prd/03-account-platform-operations.md#hizmet-isletimi) and [gözlemlenebilirlik](../../prd/15-product-quality.md#gozlemlenebilirlik). Platform proof is [platform kabulü](../../prd/16-product-acceptance.md#platform-kabulu) and [kullanılabilirlik](../../prd/15-product-quality.md#kullanilabilirlik-hedefi). Content must not grow a second local source: [ADR-0021](../../adr/0021-icerigi-yalniz-veritabaninda-tut.md). Session minting stays Account Access. No new ADR.
...
- **Online-only.** Document read/edit, record create, and planning changes require connectivity. On disconnect: show last successful save time and unsaved risk; do not enqueue, cache for write, or auto-sync on reconnect. Reconnect does not replay a hidden queue. Format times with Hesap preferences when that record exists; do not own the preference schema.
- **macOS package.** Signed with the platform certificate and notarized. Same backend and product contract as web. Tauri backend is not moved to a Rust data layer. Supported majors: current macOS major at the candidate date plus the previous two; exact versions freeze on the Ürün destek matrisi. Windows/self-host/PWA out.
...
- **Support reference.** Every failed main flow shows a clear reason, a safe retry bound, whether data was written, and a server-issued support reference with no secret and no private content. This is not a pager, S1 alarm, or operator runbook. Better Stack remains the operator sink; the user never sees tokens, emails-as-secrets, or Workspace bodies.
- **English UI labels.** First user-visible copy uses: `You’re offline`, `Last saved`, `Unsaved changes may be lost`, `Update required`, `Support reference`, `Retry`. Add missing labels to the PRD term table in the same change that first shows them.
...
- **What a good test is.** Tests observe Client Shell through its public interface: disconnect shows last save + unsaved risk and refuses writes without a queue; reconnect does not flush a hidden queue; updater accepts a valid signature and rejects an invalid one without breaking the previous app; a client past the 30-day window is stopped before a write; a failed flow returns a support reference that contains no token, session, or private field. They do not assert Tauri internals, Railway topology, or Better Stack payloads. Expected values are product rules (no queue, 30-day window, secret-free reference).
- **Seam (one).** Client Shell — the product-facing web/macOS host used by every later feature for connectivity, packaging, updater, and user-visible errors. Playwright/platform matrix observes the same seam. Signing certificates in CI may use a test double; the contract (reject invalid signature, keep previous version) is still asserted.
...
- **Orient.** Glossary: Online-only çalışma, Ürün destek matrisi, Hesap, Çalışma Alanı. Owning PRD: `docs/prd/03-account-platform-operations.md` (çalışma ve dağıtım modeli; kullanıcı hata sözleşmesi). ADRs in play: 0021 (content in the database only). Related but not owning: PRD 15 (availability SLI, observability), PRD 16 (platform kabulü), PRD 19 (no offline, no Windows, no self-host), ADR-0009 (EU region is not UI here), workflow 01 (session).
