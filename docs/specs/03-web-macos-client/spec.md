...
- **English UI labels.** First user-visible copy uses: `You’re offline`, `Last saved`, `Unsaved changes may be lost`, `Update required`, `Support reference`, `Support reference unavailable.`, `Data was written.`, `Data was not written.`, `Data write outcome is unknown.`, `Retry`, `Skip to main content`. That spec English is the name in code and in the interface.
