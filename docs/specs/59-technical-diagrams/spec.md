...
9. As a founder drawing a Data Model Diagram, I want PostgreSQL entities, fields, keys, and relations as a designed schema inside the product, so that the canvas is not bound to a live database and is not an ORM/Drizzle schema sync.
