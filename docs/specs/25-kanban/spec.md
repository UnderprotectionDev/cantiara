...
7. As a founder using `Board` or `List`, I want the in-progress Work count visible and active Work to show time-in-current-status, so that I can see load without a health score.
8. As a founder who set an optional personal focus threshold on the Project or Smart Collection, I want a visual warning in `Board` and `List` when it is exceeded, so that I notice overload without being blocked.
...
- **Soft WIP and focus.** Optional per-status soft WIP (default off, Configuration mode) and optional personal focus threshold (Project or related Smart Collection) are separate settings. `Board` and `List` both show the in-progress Work count and a focus-threshold overflow warning. Board column overflow is a neutral, not-color-only mark plus counts. Moves are not blocked. No notification, health, or performance verdict. No automatic Work mutation.
- **Card and collapse.** Cards use the saved view’s visible-fields setting with the PRD default summary, including Reappear date when it is due or has passed. Collapse is layout-only: name, count, and important signals such as open blocker remain; membership and status are unchanged. Active cards show Time in status from the status-change timestamp.
- **Order.** Kanban and ordinary Smart Collection views do not keep an independent manual card rank. They use the Project saved view’s explicit sort, shared by Board and List. The default is Work number ascending; Configuration Mode can sort by Work number, Title, Created, Updated, or Reappear date in either direction. Backlog’s one persistent manual order and a Prioritization session’s view-local rank are the two non-interchangeable exceptions, owned by those features. Named Roadmap group/column presentation order is not card priority and is not stored here.
- **List.** `List` is the same filtered Work scan as Board in a dense row layout, including unplanned Work. A row is not a second record. List does not write status or closure. It is not Table View (Record Discovery), not a Smart Collection, and not Backlog manual order.
- **Reappear date.** A future Reappear date backgrounds Work from the default Board and List set until the date in the account time zone; the due-date and past-date Work returns without a status change. The Work record owns this field.
- **Drag stack.** Column moves use the stack drag library; the product meaning is status write, not GitHub status and not silent automation.
- **English UI labels.** First user-visible copy uses: `Board`, `Kanban`, `Move`, `Status`, `List`, `Not Started`, `In Progress`, `Blocked`, `Closed`, `Completed`, `Abandoned`, `Open source record`, `Time in status`, `In Progress count`, `Collapse`, `Expand`, `Open blocker`, `Soft WIP`, `Over limit`, `Focus threshold`, `Sort by`, `Sort direction`, `Work number`, `Title`, `Created`, `Updated`, `Reappear date`, `Ascending`, `Descending`. Missing labels are added to the PRD term table in the same change that first shows them. No Turkish UI ([Kanban test seam'i](#testing-decisions)).
...
- **What a good test is.** Tests observe Kanban through its public interface: column move, closure-step on `Closed`, reopen, soft WIP/focus overflow still movable, shared in-progress count and focus warning in Board and List, List scan of the same Work, collapse, saved-view sort, elapsed current-status duration, and future Reappear date backgrounding with no independent rank. They do not assert CSS grid internals or Drizzle column maps. Expected values are product rules (status written only by column move or explicit action; Closed does not skip closure; WIP does not block; a future Reappear date does not change status).
...
- **Issue #182 card summary.** The current Project Work surface has no saved Work view field selector, so this slice uses a fixed summary: Work key/title/type, status and closure outcome, available priority metrics, planned/target/reappear dates, active blockers/Risks, and checklist progress. Tests at this seam verify priority and active blocker/Risk summaries from the existing Work Context projection.
- **Move controls.** The `Move` drag handle and `Status` control target only the four protected columns. Contract tests verify that open-column moves write the source Work status, while moves across `Closed` use the existing explicit closure or reopen step.
