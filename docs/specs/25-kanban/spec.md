...
- **English UI labels.** First user-visible copy uses: `Board`, `Kanban`, `Move`, `Status`, `List`, `Not Started`, `In Progress`, `Blocked`, `Closed`, `Completed`, `Abandoned`, `Open source record`. Missing labels are added to the PRD term table in the same change that first shows them. No Turkish UI ([Kanban test seam'i](#testing-decisions)).
...
- **Issue #182 card summary.** The current Project Work surface has no saved Work view field selector, so this slice uses a fixed summary: Work key/title/type, status and closure outcome, available priority metrics, planned/target dates, active blockers/Risks, and checklist progress. Tests at this seam verify priority and active blocker/Risk summaries from the existing Work Context projection. Reappear date and saved-view sorting remain with their owning data contracts.
