...
- **English UI labels.** First user-visible copy uses: `Board`, `Kanban`, `Move`, `Status`, `List`, `Not Started`, `In Progress`, `Blocked`, `Closed`, `Completed`, `Abandoned`, `Open source record`. Missing labels are added to the PRD term table in the same change that first shows them. No Turkish UI.
