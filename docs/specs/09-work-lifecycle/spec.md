...
- **Keys.** Internal immutable id plus user-facing `{shortCode}-{n}`. Counter is unique under concurrency; gaps allowed. Title/Project-name change does not change the key. Recreate allocates a new key. Merged Work’s key is a retired-identity redirect, not a living alias. Merge consolidates into one surviving main record; the loser does not remain a living `Closed` Work and does not receive a `Duplicate` close result. Recreate copies only founder-selected portable fields (default: title, type, description, light checklist) and portable relations; new Work opens at the target Project default start status (`Not Started`). Source status, type, and close result stay. Non-portable: GitHub completion links, automation, planning membership, publish, parentage, merge state, history, close result, current status, date-typed custom values, other absolute dates.
...
- **Stack.** oRPC, Drizzle, TanStack Form. Keys freeze Project Shell short code.

## Testing Decisions

- **What a good test is.** Tests observe Work Lifecycle through its public interface: create+key, no reuse, type matrix, status/result matrix, close cancel, Kanban cannot skip close step, reopen with the previous result retained in history, archive filter, merge preview, recreate new identity, capture conversion provenance, inclusion cardinality, Feature exit blocked while included Work/health/Primary spec remain, tree drag refused. They do not assert Kanban CSS or Drizzle schema mappings. Expected values are product rules (status ≠ result, no move, one primary Feature, and capture origin survives conversion).
- **History persistence counterpart.** The public Work profile exposes only the active closure result, so its retention after reopening is additionally verified in this seam's PostgreSQL integration test by reading the semantic Work snapshots in Mutation History (`previousValue` / `nextValue`). Keep this check scoped to closure and reopen semantics; do not assert Drizzle column mappings or unrelated storage structure.
