...
- **What a good test is.** Tests observe Test Assessments through the public interface: save snapshot with exact ids and decision, later Failed result does not rewrite decision, new-context notice lists those sources, new Project Release does not inherit the old snapshot, publish without Assessment remains allowed, no score field. Not Drizzle shapes.
