...
- **Orient.** Glossary: Proje, Proje kısa kodu, Başlangıç yapılandırması, Blank Project, Başlangıç iskeleti, Proje alanı. Owning PRD: `docs/prd/04-workspace-and-projects.md` (profil, yapılandırmalar, mod, alanlar, kopyalama). ADRs in play: none. Related: PRD 02 (protected semantics, stages), PRD 16 (İlk Proje, Başlangıç iskeletleri), PRD 19 (no contentful templates, no duplicate-project).
- **Acceptance.** Bind to [İlk Proje](../../prd/16-product-acceptance.md#uctan-uca-kabul-yolculuklari) (name + four configurations, suggested code mutable until first Work, `Active`, table applied once, Overview and All Tools always on, grouped area nav). Bind catalog selection to [Başlangıç iskeletleri](../../prd/16-product-acceptance.md#uctan-uca-kabul-yolculuklari) without stealing the five golden living-structure tests from Documents and Project Wall.
- **Conflict note.** PRD 16’s Başlangıç iskeletleri journey currently describes producing Document/Wall records. Phase-context and this workflow cut keep instantiation in 31 and 51. PRD still wins on *what* the empty headings are; *when* they materialize is the later features. Do not implement living instances here.
- **Consumers.** 08 reads enabled areas and profile. 09 consumes short code at first Work and freezes it. 10 owns field schema opened from Configuration Mode. 31/51 consume skeleton catalog selection.
