...
- **Design surfaces.** `Design` collects `Project Wall`, `Screen`, `User Flow`, and `Moodboard` as in-page destinations on that Proje alanı. One surface is shown at a time. Those names are not extra Project areas and do not leave the Design shell.
