...
20. As a founder, I want the active mode control to say `Exit Configuration Mode`, so that I can leave Configuration Mode in one action and cannot get stuck in settings.
...
- **Design in-page destinations.** `Design` collects `Project Wall`, `Screen`, `User Flow`, and `Moodboard` as in-page destinations on that Proje alanı. One destination is shown at a time. Those names are not extra Project areas and do not leave the Design shell.
...
- **English UI labels.** First user-visible copy uses: `Project Name`, `Starter Configuration`, `Blank Project`, `Solo SaaS`, `Open Source Library`, `Mobile Application`, `Configuration Mode`, `Exit Configuration Mode`, `Overview`, `All Tools`, `Pin to navigation`, `Restore default navigation`, `Navigation preview`, `Current pinned areas`, `Default pinned areas`, `Copy project structure`, `Short code`, `Not Planned`, `Ready`, `Custom field`, `Work Context Card layout`, `Priority metrics`, `Stages`, `Work statuses`, `Project areas`, `Saved views`, `Create`, `Edit`, `Status`, `Planning`, `Design`, `Project Wall`, `Screen`, `User Flow`, `Moodboard`. Add missing labels to the PRD term table in the same change that first shows them. `Starter Configuration` and lifecycle `Active` are already in the table. Stage `Active` / `Completed` / `Abandoned` reuse existing lifecycle labels and are not a second Work-status catalog.
- **Configuration Mode.** Visible presentation state on the Project Shell, not a permission and not a Mutation Contract write. One control opens as `Configuration Mode` and closes as `Exit Configuration Mode`. Entering does not change records, view membership, or Project lifecycle. Daily `Create`, `Edit`, `Status`, and `Planning` stay available outside the mode. The mode hosts stages, Work statuses, Project areas, Custom field, Priority metrics, Saved views, and Work Context Card layout; it also opens the Work Template host from 17. Saved views is not the daily Planning action. Custom field and Work Context Card layout editors open as hosts; schema and layout engines stay 10 and 16. The Work Template editor opens as a host; template payload and its writes stay 17.
- **Stack.** React, TanStack Router/Form, Zod, oRPC, Drizzle. No new framework.

## Testing Decisions

- **What a good test is.** Tests observe Project Shell through its public interface: create with name+configuration; short-code reserve and immutability after first Work (Work create may be a test double that only “exists”); four-configuration matrix; no sample content; Configuration Mode opens with `Configuration Mode`, closes with `Exit Configuration Mode`, and changes no records, lifecycle, enabled areas, or view membership; daily Create/Edit/Status/Planning remaining available with the mode off; Custom field and Work Context Card layout editors opening as hosts without schema or layout writes, and the Work Template host opening without template payload writes; hide area without deleting; Overview/All Tools still reachable; Overview modules absent; stage five-state catalog; copy structure without records; skeleton catalog metadata without living Document/Wall; Design in-page destinations stay `Project Wall`, `Screen`, `User Flow`, and `Moodboard` without becoming extra Project areas. They do not assert Drizzle shapes or CSS. Expected values are the PRD table and the closed catalogs.
