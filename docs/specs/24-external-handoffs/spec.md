...
- **Going package.** Markdown is generated only from the user-selected exact version manifest. It includes production time, Work key, handoff identity, and `Source of truth is in the app`. Secret fields, inaccessible fields, and unselected related records are not selected. User-provided free text and selected Work title/description are copied as entered; the package preview states that free text is not scanned for secrets and asks the founder to review before sharing. After send, later source edits do not mutate the sent bytes; the founder creates a new package version on the same handoff or starts a new handoff.
- **Issue #166 slice boundary.** This issue selects the current Work revision and permitted GitHub identifiers supplied by the founder. Document, Decision, Risk, Open Question, and Source versions are deferred until those source types have versioned contracts; this slice does not add their record or version foundations. Return, reconcile, cancellation, and attention signals remain later handoff slices.
...
- **English UI labels.** First user-visible copy uses: `External Execution Handoff`, `Start Handoff`, `View Handoffs`, `Handoff history`, `Handoff started`, `Going package produced`, `Going package copied`, `You`, `Free text is copied as entered and is not scanned for secrets. Review the package before sharing.`, `Reconcile`, `Cancel Handoff`, `Open`, `Result returned`, `Reconciled`, `Canceled`, `Source of truth is in the app`. Missing labels are added to the PRD term table in the same change that first shows them. No Turkish UI.
- **History.** Handoff start, package production, and package export are separate ordinary Work change-history events with actor and time. The Work owner sees their own events attributed as `You`. History values contain only the event type and handoff identity; package text, free text, tokens, and secrets never enter those events. They are not a second Denetim kaydı class. Return, reconcile, and cancel follow the same rule in their later slices.

## Testing Decisions

- **What a good test is.** Tests observe External Execution Handoff through its public interface: start on a Work, package bytes from a selected version manifest, start/production/export Work-history events with actor and time and visible `You` attribution but no package content, archived Work history reads, the free-text warning, record return, reconcile preview/confirm, cancel with reason, and whether `external-run-returned` is produced or closed. They do not assert Drizzle row shapes, Markdown template internals, or mock private collaborators. Expected values are product rules (owned component, frozen package, terminals only `Reconciled`/`Canceled`, no silent field overwrite).
- **Issue #166 coverage.** The start-package contract covers the current Work revision when selected, omission when not selected, permitted GitHub identifiers, frozen Markdown, and strict rejection of unsupported source-version fields. It does not claim Document, Decision, Risk, Open Question, or Source version selection.
...
- **Acceptance.** For Issue #166, bind tests to starting an owned Work handoff, selecting the current Work revision and permitted GitHub identifiers, keeping its Markdown package frozen, and not starting an external runner. For Issue #167, bind tests to two repeated handoffs on one Work, preserving the first package after a source edit, and distinct actor/time history events for handoff start and package production. The broader acceptance journey for late return, cancel, reconcile, signals, and Test Handoff separation remains future feature work; this slice does not claim it complete.
