...
16. As a founder, I want English UI copy for `Calendar`, `Day`, `Week`, `Month`, `Agenda`, `Planned start`, `Target date`, `Reappear date`, and `Selected day`, so that the product language stays English.
...
- **Day / week / month.** Range rendering: start+target together → range in week/month on each calendar day the span covers, so duration is visible; day view shows only positions on the selected day and does not paint the range onto that day. Planned start does not hide, auto-start, or write status.
...
- **English UI labels.** First user-visible copy uses: `Calendar`, `Day`, `Week`, `Month`, `Agenda`, `Planned start`, `Target date`, `Reappear date`, `Open source record`, `Selected day`. Missing labels are added to the PRD term table in the same change that first shows them. No Turkish UI.

## Testing Decisions

- **What a good test is.** Tests observe Unified Calendar through its public interface: kind-separated day/week/month/Agenda, range vs day positions, a start–target range present on each spanned day in week/month, drag of one represented field with preview, status unchanged, no Event record. They do not assert calendar-grid internals. Expected values are product rules (kinds do not mix; drag writes one field; Agenda is not an Event).
