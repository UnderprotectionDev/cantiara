...
13. As a founder producing the Markdown/JSON package, I want only the closed package contract: explicit `schema_version` (`test-handoff-package/1`), `handoff_id` with incrementing `handoff_package_version`, project, title/purpose, `created_at`, filled `product_build_context` fields only, `scenarios[]` with id+version and copied intent fields, `ad_hoc_scope[]` as my text, selected work/document/design refs as exact versions, `environment_preconditions` as my text, and `return_instructions` naming `test-report/1`.
...
- **Planned Test Case ilişkileri.** Bu seam, canlı resolver'larla mevcut ve Proje'ye ait ilişki hedeflerini sunar: `Work` (`Feature` work dahil), kesin revision ve isteğe bağlı kararlı section ile `Document`, `Decision`, `Risk`, `Project Goal`, `Milestone`, `Source` ve `Screen`. `Project Release` ile GitHub dış kayıtları specs 63 ve 61 sahiplenir; bu feature henüz oluşturulmamış kayıtlar için placeholder id üretmez. Repository, branch ve commit değerleri yürütme bağlamı olduklarında kurucunun yazdığı `product_build_context` içinde kalır; ilişki hedefi değildir.
...
- **What a good test is.** Tests observe Planned Tests and Handoff through the public interface: case versioning, session bind pinned to selected version (historical bind not moved), Handoff states, Markdown/JSON equality for one package version, explicit root package `schema_version`, closed package membership, no runner API, package vs Work external-handoff contract difference. Expected values are the PRD table, not regenerated fixtures from code.
- **İlişki hedefi testi.** Planned Tests seam'i, Proje kapsamındaki hedef listesini ve bu feature'ın sahip olduğu her hedef ailesi için çözümlenmiş bir ilişkiyi sınar. Sahip kaydı veya resolver'ı olmayan hedef reddedilir; yalnız başlık ya da placeholder ilişki kabul edilmez.
