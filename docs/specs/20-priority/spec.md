...
- **English UI labels.** `Very low`, `Low`, `Medium`, `High`, `Very high`, `Evidence strength`, `Priority Map`, `Unevaluated`, `Create Prioritization Session`, `Add metric`, `Short description`, `Rank descriptions`, `Disable`. Add missing labels to the term table in the same change.
...
- **What a good test is.** Tests observe Prioritization through its public interface: criterion CRUD and ranks, exact English editor labels, map placement including Unevaluated, session order versus Backlog, and negatives (no score, no auto-fill Evidence strength, no implicit sync). They do not assert numeric rollups that the product must not have.
