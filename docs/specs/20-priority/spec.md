...
- **What a good test is.** Tests observe Prioritization through its public interface: criterion CRUD and ranks, independent identities when Project structure copies definitions, same-key copy retries returning the prior result and changed-payload conflicts through Mutation Contract, exact English editor and confirmation labels, map placement including Unevaluated, session scope add/remove, session order versus Backlog, Backlog reorder persistence, and negatives (no score, no auto-fill Evidence strength, no implicit sync). They do not assert numeric rollups that the product must not have.
