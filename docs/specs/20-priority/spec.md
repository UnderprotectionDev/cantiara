...
- **English UI labels.** `Very low`, `Low`, `Medium`, `High`, `Very high`, `Evidence strength`, `Priority Map`, `Unevaluated`, `Horizontal axis`, `Vertical axis`, `Show evidence signals`, `Edit axis values`, `Create Prioritization Session`, `Add metric`, `Short description`, `Rank descriptions`, `Disable`, `Disabled`, `Type the Project name to confirm`. Add missing labels to the term table in the same change.
...
- **What a good test is.** Tests observe Prioritization through its public interface: criterion CRUD and ranks, independent identities when Project structure copies definitions, same-key copy retries returning the prior result and changed-payload conflicts through Mutation Contract, exact English editor and confirmation labels, map placement including Unevaluated, the default-off context-only evidence signal, an explicit axis-value edit, session order versus Backlog, and negatives (no score, no auto-fill Evidence strength, no implicit sync or map-position writes). They do not assert numeric rollups that the product must not have.
