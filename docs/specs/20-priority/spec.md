...
- **English UI labels.** `Very low`, `Low`, `Medium`, `High`, `Very high`, `Evidence strength`, `Priority Map`, `Unevaluated`, `Create Prioritization Session`, `Add metric`, `Short description`, `Rank descriptions`, `Disable`, `Disabled`, `Type the Project name to confirm`. Add missing labels to the term table in the same change.
