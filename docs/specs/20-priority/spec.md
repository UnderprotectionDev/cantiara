...
- **Map.** Two ordered criteria as axes; the first two enabled criteria are selected initially and the founder can change either axis. Missing values in `Unevaluated`. The optional `Show evidence signals` control starts off and shows Feedback and unique Contact/Company counts beside each point as context only. Map does not combine axes, auto-rank, impose quadrant labels, or change the founder's priority. Points do not drag; `Edit axis values` is the explicit action that writes the same criterion field. Axis selection is temporary view state; saving the map is not a second ranking truth. Map is not an analytics dashboard or publish score.
...
- **Backlog order.** Backlog has its own Project-scoped persistent order, manually changed with `Move up` and `Move down`. New active Work is appended in stable Work-number order. Archived Work is hidden from the active order while its stored position is preserved. Session scope and order never write this value; Backlog moves never rewrite a session.
- **Trash.** Criteria, named views, and sessions follow configuration trash (PRD 13). This feature does not build global trash UI.
- **English UI labels.** `Very low`, `Low`, `Medium`, `High`, `Very high`, `Evidence strength`, `Priority Map`, `Unevaluated`, `Horizontal axis`, `Vertical axis`, `Show evidence signals`, `Edit axis values`, `Prioritization`, `Prioritization Sessions`, `Create Prioritization Session`, `Session name`, `Work scope`, `Work to add`, `Select Work`, `Add Work`, `Remove from session`, `Move up`, `Move down`, `Close session`, `Session order`, `Backlog order`, `Manual order`, `Target date`, `Risk`, `Evidence`, `Trash`, `Restore`, `Add metric`, `Short description`, `Rank descriptions`, `Disable`, `Disabled`, `Type the Project name to confirm`. Add missing labels to the term table in the same change.
...
- **What a good test is.** Tests observe Prioritization through its public interface: criterion CRUD and ranks, independent identities when Project structure copies definitions, same-key copy retries returning the prior result and changed-payload conflicts through Mutation Contract, exact English editor and confirmation labels, map placement including Unevaluated, the default-off context-only evidence signal, an explicit axis-value edit, session scope add/remove, session order versus Backlog, Backlog reorder persistence, and negatives (no score, no auto-fill Evidence strength, no implicit sync or map-position writes). They do not assert numeric rollups that the product must not have.
...
- **Required counterparts.** No scalar priority field; Project structure copy creates independent criteria and skips trashed definitions, with same-key retries returning one copy; map reorder of points without explicit edit does not write; session order ≠ Backlog; Backlog order persists independently; add/remove session scope changes neither Work status nor Backlog membership; Evidence strength not auto-computed; closing keeps the dated last order read-only; trash/restore leaves Work and Backlog unchanged.
