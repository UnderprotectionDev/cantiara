...
- **Values storage.** Definitions and values live in Drizzle. A value is stored per (definition, record); the absence of a value row is the `Not evaluated` state, which keeps empty distinct from a Boolean false or an empty selection. Date values are calendar dates (`YYYY-MM-DD`) and are never rewritten into zoned instants. Setting and clearing a value is a Mutation Contract write whose target is the (definition, record) pair and whose base revision is the stored value revision. Unbinding a type keeps stored values; trashing a definition keeps values while hiding the field from record surfaces.
...
- **English UI labels.** First user-visible copy uses: `Custom field`, `Text`, `Number`, `Boolean`, `Date`, `Single select`, `Multi select`, `Field name`, `Type`, `Options`, `Record types`, `Custom fields`, and `Add custom field`. Add missing labels to the PRD term table in the same change that first shows them.
- **Stack.** Drizzle definition+value, Zod validation, TanStack Form, React DayPicker for Date. No formula engine.
...
- **UI copy at the seam.** The Configuration Mode editor exposes the labels `Custom field`, `Field name`, `Type`, `Options`, `Record types`, `Custom fields`, and `Add custom field`; the Playwright acceptance path checks these controls together with the closed type and binding catalogs.
