...
- **English UI labels.** `Smart Collection`, `List`, `Table`, `Gallery`, `Insights`, `Subscribe`, `Notify on leave`, `Turn on Subscribe first.`, `Purpose`, `New work`. Missing labels join the PRD term table in the same change that first shows them. No Turkish UI.
