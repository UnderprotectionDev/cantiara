...
- **Indexes.** Zero-setup, type selector, not per-type main nav. Closed list: `All Work`, `All Documents`, `All Decisions`, `All Risks`, `All Research Sessions`, `All Tests`, `All Designs`, `All Technical Diagrams`, `All Project Releases`, `All Sources`, `All Files`. `All Tests` keeps Planned Test Case, Test Handoff, Test Session, Session Test, Test Gap, and Test assessment distinct. `All Technical Diagrams` keeps Technical Architecture, Data Model, Technical Sequence, and Diagram authority mode distinct. Archive stays findable with filter; not treated as deleted. Document and File Attachment indexes use the same prepared surface with scope, type, folder, and supported metadata; folder is navigation, not ownership, and does not replace Universal Search.
- **Table matrix.** Honor the closed type × surface matrix (PRD 08): Work, Project Goal, Milestone, Project Release, Feedback, Contact, Company, User Research Session, Decision, Risk, Assumption, Open Question, Product Gap, Source, Planned Test Case, Test Handoff, Test Session, Test Gap, Test assessment, Production Incident → Table+inline and Smart Collection source. Document/Wiki Document → search/index yes, Table+inline no, Smart Collection source on structured metadata/tags/scope only. Screen, User Flow, Project Wall, Moodboard, Technical Diagram, File Attachment → search/index yes, Table+inline no, Smart Collection source no. Capture Inbox item, Draft, External Surface, GitHub external record → not in Search/Table/collection source. Inline edit is not bulk edit (22). Session Test results are not written from the cell except via the test-correction event owned elsewhere.
