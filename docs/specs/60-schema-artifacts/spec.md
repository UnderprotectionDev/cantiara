...
- **Required counterparts.** Live migrate absent; invented Down absent; modeling commands absent (59); Drizzle Kit migrations are the product's own schema story, not this user artifact.

## Out of Scope

- Veri Modeli canvas modelleme — 59.
- Canlı/staging/production migrate, credential, introspection, drift.
- Drizzle runtime, veri backfill, `USING` serbest tip, view/function/trigger kataloğu.
- Artefaktı repository'ye yazma veya PR açma — 19.
- Ürünün kendi `drizzle-kit migrate` kabulü (PRD 16 migration sınırı); kullanıcı artefaktı ile karıştırılmaz.
