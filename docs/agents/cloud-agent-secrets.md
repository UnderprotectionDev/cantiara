...
`db push` during install/start always targets local Postgres, not hosted Neon. After adding a Prisma model, apply that schema to the hosted product `DATABASE_URL` before the API is used (`cd packages/db && bunx prisma db push` with the injected URL). A bun `--hot` process started before generate still holds a Prisma client without the new delegate; restart that API process after generate.
