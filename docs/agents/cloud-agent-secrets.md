...
Prisma Client is generated with `bunx --bun prisma generate` (Prisma CLI shebang is Node; `--bun` keeps the generator on Bun). Cloud `start.sh` and `run-dev.sh` generate after checkout. `run-dev.sh` then runs `prisma migrate deploy` against hosted `DATABASE_URL` (Neon `-pooler` host stripped to the direct endpoint; Prisma migrate needs a session connection). Parallel agent worktrees sharing one Neon project stay in sync because deploy is additive and idempotent. Local loopback URLs stay on `db push` in `start.sh`. After generate, `run-dev.sh` replaces leftover listeners on 3000/3001/4000 from a warm snapshot. `getPrismaClient()` keeps one client on `globalThis` in development (Prisma connection-management docs) and replaces it when the generated class file on disk changes, so `bun --hot` does not keep a client from before generate.
